...
export default function UGCRewardsPage() {
  ...
  // Check localStorage for previously issued coupon for this user
  useEffect(() => {
    const savedCode = localStorage.getItem('KULT_UGC_COUPON');
    if (savedCode) {
      setGeneratedDiscountCode(savedCode);
    }
  }, []);
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const emailInput = formData.customerId.trim().toLowerCase();

      // One-time coupon validation check via localStorage
      const existingEmail = localStorage.getItem('KULT_UGC_CLAIMED_EMAIL');
      if (existingEmail && existingEmail === emailInput) {
        throw new Error('LIMIT EXCEEDED: A reward coupon has already been claimed for this email account.');
      }
      ...
      if (selectedFile) {
        ...
        if (uploadRes.ok) {
          ...
          throw new Error(
            uploadData.errors?.[0]?.message || 'Media upload failed. Check file size/format.'
          );
        }
      ...
      // Step 2: Post Review record to API
      const response = await fetch('/api/ugc-reviews', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          customerId: emailInput,
      ...
      if (response.ok && result.success) {
        if (result.discountCode) {
          ...
          // Lock customer email and code in localStorage to restrict duplicate claims
          localStorage.setItem('KULT_UGC_COUPON', result.discountCode);
          localStorage.setItem('KULT_UGC_CLAIMED_EMAIL', emailInput);
        }
      ...
      <div className="relative z-20 pb-16 sm:pb-20">
        ...
        <header className="pt-6 sm:pt-8 px-4 sm:px-6 max-w-7xl mx-auto">
          <div className="flex justify-between items-center max-w-5xl mx-auto gap-2">
            ...
            >
              <span className="text-[#E8E2D6] group-hover:text-[#D4AF37] group-hover:-translate-x-1 transition-all duration-300 text-xs sm:text-sm">
                ←
              </span>
        ...
        <section className="pt-8 sm:pt-10 pb-10 sm:pb-12 px-4 sm:px-6 max-w-7xl mx-auto border-b border-[#2D323E]/60 space-y-6 sm:space-y-8">
          <div className="space-y-3 sm:space-y-4 text-center max-w-4xl mx-auto">
            ...
            <p className="text-[11px] sm:text-sm text-[#E8E2D6]/90 uppercase tracking-widest max-w-2xl mx-auto leading-relaxed font-mono">
              SUBMIT WRITTEN, PHOTO, OR VIDEO VALIDATION. CLAIM UP TO 20% DISCOUNT (ONE-TIME CLAIM PER OPERATIVE).
            </p>
          </div>
        </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 sm:gap-6">
            ...
            >
              ...
              <p className="text-[10px] sm:text-[11px] text-[#E8E2D6]/70 uppercase tracking-wider mt-2 sm:mt-3 leading-relaxed">
                Written feedback detailing fabric & fit performance under tension. No file required.
              </p>
            </div>
          ...
          {!submissionSuccess ? (
            ...
            >
              <div className="flex justify-between items-center border-b border-[#2D323E] pb-3 sm:pb-4">
                ...
                <span className="text-[9px] sm:text-[10px] text-[#E8E2D6]/40 uppercase tracking-widest">
                  PAYLOAD CMS STORAGE
                </span>
              </div>
                  ...
                  <label htmlFor="ugc-file-input" className="cursor-pointer space-y-1.5 sm:space-y-2 block">
                    <p className="text-xs text-[#E8E2D6] font-bold uppercase tracking-widest hover:text-[#D4AF37] transition-colors break-all">
                      {selectedFile
                        ...
                        : `ATTACH ${reviewType === 'TEXT' ? 'MEDIA (IF ANY)' : reviewType} FILE HERE`}
                    </p>
              ...
              >
                <span className="group-hover:text-[#D4AF37] transition-colors">
                  {isSubmitting
                    ? 'SAVING TO PAYLOAD DATABASE...'
                    : `TRANSMIT REVIEW FOR ${targetDiscountPercentage}% DISCOUNT VALIDATION`}
                </span>
              </button>
            </form>
          ) : (
            <div className="bg-[#12141B]/95 backdrop-blur-md border border-[#2D323E] p-6 sm:p-10 rounded-2xl text-center space-y-5 sm:space-y-6 shadow-[0_20px_50px_rgba(0,0,0,0.95)]">
              <div>
                <span className="text-[10px] sm:text-xs text-[#D4AF37] font-bold uppercase tracking-[0.2em] border border-[#D4AF37]/40 px-4 sm:px-5 py-1.5 rounded-full inline-block bg-[#0A0B0D]">
                  REVIEW RECORDED // STATUS: DISCOUNT ALLOCATED
                </span>
              </div>
              ...
              {generatedDiscountCode && (
                <div className="bg-[#0A0B0D] border border-[#D4AF37]/50 rounded-xl p-4 my-4 max-w-md mx-auto">
                  <p className="text-[10px] text-[#D4AF37] uppercase tracking-widest mb-1">
                    YOUR EXCLUSIVE DISCOUNT CODE
                  </p>
                  <p className="text-xl sm:text-2xl font-black text-[#E8E2D6] tracking-widest select-all">
                    {generatedDiscountCode}
                  </p>
                </div>
              )}
              ...
              <p className="text-[11px] sm:text-xs text-[#E8E2D6]/70 uppercase tracking-widest max-w-md mx-auto leading-relaxed">
                YOUR REVIEW HAS BEEN STORED IN PAYLOAD CMS. USE YOUR PROMO CODE ({targetDiscountPercentage}% DISCOUNT) AT CHECKOUT.
              </p>

              <div className="pt-2 sm:pt-4 flex justify-center">
                <Link
                  href="/checkout"
                  className="group w-full sm:w-auto py-3.5 sm:py-4 px-8 sm:px-10 bg-[#D4AF37] text-black font-black text-[11px] sm:text-xs uppercase tracking-[0.2em] rounded-full transition-colors shadow-[0_4px_25px_rgba(212,175,55,0.35)] backdrop-blur-md active:scale-95"
                >
                  GO TO CHECKOUT TO APPLY DISCOUNT
                </Link>
              </div>
            </div>
          )}
